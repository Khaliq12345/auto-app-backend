...
def numeric_to_alphabetic_column_name(
    n: int,
) -> str:
    if n < 0:
        raise ValueError(
...
def map_equipment(
    row_raw_dict: dict, key: str
) -> bool | None:
    try:
        return (
            True
            if int(row_raw_dict.get(key, 0)) == 1
            else False
        )
...
def get_row_dict(df: pd.DataFrame, row_id: int):
    ...
    row["price_with_tax"] = (
        float(row_raw_dict["K"]) * 1.19
    )
    row["boite_de_vitesse"] = (
        int(row_raw_dict.get("DG"))
        if row_raw_dict.get("DG")
        else None
    )
    ...
    row["camera_360"] = map_equipment(
        row_raw_dict, "JY"
    )
    row["attelage"] = map_equipment(
        row_raw_dict, "AI"
    )
    row["bluetooth"] = map_equipment(
        row_raw_dict, "FS"
    )
    row["aide_stationnement_avant"] = (
        map_equipment(
            row_raw_dict,
            "GR",
        )
    )
    row["radar_de_recul"] = map_equipment(
        row_raw_dict, "GS"
    )
    row["camera_de_recul"] = map_equipment(
        row_raw_dict, "GT"
    )
    row["kit_mains_libres"] = map_equipment(
        row_raw_dict, "FX"
    )
    row["limiteur_de_vitesse"] = map_equipment(
        row_raw_dict, "KX"
    )
    row["affichage_tete_haute"] = map_equipment(
        row_raw_dict, "FW"
    )
    row["gps"] = map_equipment(row_raw_dict, "AL")
    row["toit_panoramique"] = map_equipment(
        row_raw_dict, "GD"
    )
    row["toit_ouvrant"] = map_equipment(
        row_raw_dict, "AM"
    )
    row["sieges_chauffants"] = map_equipment(
        row_raw_dict, "CX"
    )
    row["assistant_changement_voie"] = (
        map_equipment(row_raw_dict, "IV")
    )
    row["assistant_maintien_voie"] = (
        map_equipment(row_raw_dict, "IQ")
    )
    cuir_value = row_raw_dict.get("FA")
    row["cuir"] = (
        True if cuir_value == 1 else False
    )

    four_by_four = row_raw_dict.get("AP")
    row["4x4"] = (
        True if four_by_four == 1 else False
    )
...
def parse_and_save(
    car_dict: dict, cars: list[Car]
):
    car_dict["updated_at"] = (
        datetime.now().isoformat()
    )
    if str(car_dict["fuel_type"]).isdigit():
        car_dict["fuel_type"] = fuel_types[
            car_dict["fuel_type"]
        ]
    ...
    compared_car_dicts = drop_duplicate_cars(
        jsoned_cars
    )
    print(
        f"Total cars; {len(compared_car_dicts)}"
    )
    if compared_car_dicts:
        save_to_db(car_to_save_dict, "Vehicles")
        save_to_db(
            compared_car_dicts, "comparisons"
        )
